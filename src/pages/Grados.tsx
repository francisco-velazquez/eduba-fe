...
export default function Grados() {
  ...
  const GradoCard = ({ grado, index }: { grado: AppGrade; index: number }) => {
    const isActive = grado.isActive;

    return (
        <div
          className={`stat-card group animate-fade-in p-4 md:p-6 ${isActive ? "bg-card border-border shadow-card hover:shadow-elevated" : "bg-slate-200/80 dark:bg-slate-700/80 border-slate-400 dark:border-slate-500"}`}
          style={{ animationDelay: `${index * 50}ms` }}
        >
          <div className="flex items-start justify-between">
            <div>
              <h3 className="font-semibold text-foreground text-base md:text-lg">{grado.name}</h3>
              <p className="text-xs md:text-sm text-muted-foreground mt-1">{grado.level}</p>
            </div>
            <DropdownMenu>
          ...
          <div className="mt-4 md:mt-6 flex items-center gap-4 md:gap-6">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-lg md:text-xl font-semibold text-foreground">{grado.studentsCount}</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-lg md:text-xl font-semibold text-foreground">{grado.subjectsCount}</p>
